from doors.hip.basic_hip import BasicHip
from typing import NewType

from doors.hip.hip789 import worm

Move = NewType("Move", str)

wait = Move("wait")
a = Move("a")
b = Move("b")
c = Move("c")
d = Move("d")
e = Move("e")
f = Move("f")
sto = Move("sto")
obs = Move("obs")
TS = Move("TS")
fold = Move("fold")


class HipSeq10(BasicHip[Move]):
    piston_stack_depth = 6
    max_obs = 3

    fold_toggle = False  # true -> false triggers fold
    a_toggle = False  # true -> false is slow

    fpull_primed = False
    last_obs_is_block = False

    tape_ready_mode = False
    in_row_10 = False
    no_fold_mode = False

    def the_whole_shebang(self):
        self.closing()
        self.stop()
        self.opening()
        assert not self.fold_toggle
        assert not self.a_toggle

    def closing(self):
        # fmt: off
        self += [
            sto, b, a,
            sto, b, worm, worm,
            sto, b, a, sto, b,
            fold, f, worm, worm,
            e, d, fold, worm, worm,
            fold, fold, d, c, c, b,
            sto, b, a, sto, b,
            f, fold, worm, worm,
            a, sto, b,
            TS, b, a, sto,
            worm, worm, b, d, b, a,
            fold, fold, fold, f, d, c]
        # fmt: on
        self.stack_state[0] = True

    def stop(self):
        self += [wait] * ((26 - len(self.moves) % 26) % 26)
        self += [wait] * 26

    def storage_moves(self, *moves: Move):
        self.call_tree.add_message("-b")
        assert self.moves[-1] == b
        self.moves.pop()
        self += list(moves)

    def _fold2(self):
        if self.moves[-2:] == [fold, fold]:
            del self.moves[-2:]
            if self.moves[-1] == wait:
                self.moves.pop()
            self.call_tree.add_message("-fold2")
            return
        assert not self.fold_toggle
        self += [fold, fold]

    def opening(self):
        self += a
        self.retract(1)
        self += sto

        self.full_row(2)
        self.storage_moves(a, b, sto)

        for row in [3, 4]:
            self.full_row(row)
            self.storage_moves(a, sto)

        self.full_row(5)
        self.storage_moves(a, sto)
        self += [d, fold, fold, fold]
        self += [worm, worm, fold, f]

        self.full_row(6)
        self.storage_moves(a, sto)
        self += [d, fold, fold, fold]
        self += [worm, worm, fold]

        for row in [7, 8, 9]:
            self.full_row(row)
            self.storage_moves(a, sto)

        self.full_row(10)
        last_4 = self.moves[-4:]
        assert last_4 == [a, c, b, b]
        self.moves[-4:] = [c, b]

    def more_pistons(self, layer: int):
        if any(self.stack_state[:layer]):
            raise ValueError(f"not all layers below {layer} are False.")
        if self.stack_state[layer]:
            raise ValueError(f"layer {layer} is already True.")

        self.stack_state[layer] = True

        match layer, self.no_fold_mode:
            case 0, _:
                self += c
            case 1, _:
                self += d
            case (2, False):
                self += d
                self._fold2()
            case (3, False) | (2, True):
                self += e
            case (4, False) | (3, True):
                assert not self.fpull_primed
                self += f
            case 5, False:
                self.fpull_primed = True
                self += f
            case _:
                raise NotImplementedError

    def shift_pistons(self, layer: int):
        if layer == 5:
            assert self.stack_state == [False, False, False, False, True, False]
            return

        if not self.stack_state[layer]:
            raise ValueError(f"layer {layer} not out")
        self.stack_state[layer] = False

        if layer > 0:
            if self.stack_state[layer - 1]:
                raise ValueError(f"layer {layer - 1} is already out.")
            self.stack_state[layer - 1] = True

        match layer, self.no_fold_mode:
            case 0, _:
                self += [c, b]
            case 1, _:
                self += [d, c]
            case 2, False:
                self._fold2()
            case 3, False:
                self += [e, d]
                self._fold2()
            case 2, True:
                self += [e, d]
            case (4, False) | (3, True):
                self += [f, e]
            case (4, True):
                self.call_tree.add_message("(start no fold mode")
                pass
            case _:
                raise NotImplementedError

        if layer == 3 and self.tape_ready_mode:
            self.tape_in_the_phokin_piston_stack()
            self.tape_ready_mode = False

    def more_obs(self):
        if self.num_obs_out >= self.max_obs:
            raise ValueError("Not enough observers.")
        self.num_obs_out += 1
        match self.num_obs_out:
            case 1:
                self += [obs, obs]
            case 2:
                self += [obs, a, obs]
            case 3:
                self += [sto]
                self.last_obs_is_block = True
            case _:
                raise NotImplementedError

    def less_obs(self):
        if self.num_obs_out <= 0:
            raise ValueError("Retracting more observers than there are.")
        self.num_obs_out -= 1
        if self.last_obs_is_block:
            self += [b, sto]
            self.last_obs_is_block = False
            return
        match self.num_obs_out:
            case 0 | 1:
                self += [sto, sto]
            case _:
                raise NotImplementedError

    def full_row(self, layer: int):
        """
        Does full row.
        _____B -> B_____
        """
        if layer == -2:
            return
        if layer == 10:
            self.in_row_10 = True
        if layer >= 0:
            self.more_pistons(layer // 2)
        self.row_high(layer)
        if layer == 10:
            self.in_row_10 = False

    def row_high(self, layer: int, pistons_high: bool = False):
        """
        Does row, but with layer of pistons already up.
            _P___B -> B_____
        """
        self.full_extend(layer, pistons_high, no_row_2_opt=True)
        self.retract(layer)

    def full_extend(
        self, layer: int, pistons_high: bool = False, no_row_2_opt: bool = False
    ):
        self.extend(layer, pistons_high, no_row_2_opt=no_row_2_opt)
        self.extra_pulses(layer, pistons_high)

    def extend(
        self, layer: int, pistons_high: bool = False, no_row_2_opt: bool = False
    ):
        match layer:
            case -1:
                self += b
            case 0:
                self += a
            case 1:
                if not pistons_high:
                    self += b
                self += a
            case 2 if not no_row_2_opt:
                if not pistons_high:
                    self += b
                self += [sto, b]
                self.last_obs_is_block = True
                self.num_obs_out += 1
            case _ if 2 <= layer < 9:
                if not pistons_high:
                    self += b
                self.more_obs()
                if layer >= 3:
                    self.more_pistons((layer - 3) // 2)
                self.extend(layer - 3)
            case 9:
                if not pistons_high:
                    self += b
                self.more_obs()  # deploy sandwich
                self.more_pistons((layer - 3) // 2)
                self += [b, sto, b]  # block
                self.more_obs()
                self.more_pistons(2 // 2)
                self += b
                self.more_obs()
                self += b
            case 10:
                assert not pistons_high
                self += [b, sto, b]  # block
                self.more_obs()
                self.more_pistons((layer - 3) // 2)
                self += [b, sto, b]  # block
                assert self.fpull_primed
                self.fpull_primed = False
                self += [worm, worm]
                self.more_obs()
                self.more_pistons(2 // 2)
                self += [fold, worm, fold, fold, fold]
                self += b
                self.more_obs()  # actually a block
                self += b
            case _:
                raise NotImplementedError

    def extra_pulses(self, layer: int, pistons_high: bool = False):
        match layer, pistons_high:
            case (-1, _) | (0, _) | (1, _):
                return
            case 2, _:
                if self.last_obs_is_block:
                    self += a
                else:
                    self += b
            case 3, _:
                self += a
            case 4, _:
                self += a
            case 5, False:
                if self.last_obs_is_block:
                    self += [a] * 2
                else:
                    self += [b] * 5
            case 5, True:
                if self.last_obs_is_block:
                    self += [a] * 2
                else:
                    self += [b] * 3
            case 6, False:
                self += [a] * 7
            case 6, True:
                self += [a] * 3
            case 7, False:
                self += [a] * 7
            case 7, True:
                self += [a] * 3
            case 8, False:
                self += [a] * 8
            case 8, True:
                self += [a] * 4
            case 9, False:
                self += [a] * 8
            case 10, False:
                self += [a] * 8
            case 9, True:
                self += [a] * 4
            case _, _:
                raise NotImplementedError

    def retract(self, layer: int, no_recurse_first=False):
        if layer <= -1:
            return

        if layer == 9:
            # special retraction
            assert self.num_obs_out == 3
            self.retract(2)  # piston-obs
            self.less_obs()  # first obs
            self.full_row(3)  # remove the sandwich
            self.storage_moves(a, b, sto)  # and eat it. Special because air?
        elif layer == 10:
            assert self.num_obs_out == 3
            self.retract(2)  # piston-obs
            self.less_obs()  # first obs
            self.full_row(3)  # remove the sandwich
            self.storage_moves(a, sto)  # and eat it
            self.retract(layer - 4, no_recurse_first=layer >= 9)
            self.less_obs()  # first obs
            self.full_row(7)
            self.storage_moves(a, sto)  # and eat it
            self.pull(layer - 2)
            if layer % 2 == 0:
                self.shift_pistons(layer // 2)
            self.row_high(layer - 1, True)
            return

        if layer >= 3 and not no_recurse_first:
            self.retract(layer - 3, no_recurse_first=layer >= 9)

        if layer >= 2 and not no_recurse_first:
            self.less_obs()

        self.pull(layer - 2)

        if self.in_row_10 and layer == 8:
            self.no_fold_mode = True

        if layer % 2 == 0:
            self.shift_pistons(layer // 2)

        self.row_high(layer - 1, True)

    def pull(self, layer: int):
        """
        Pulls pistons at row n-1 down.
        _____P -> ____P_
        """

        if layer == -2:
            return
        if layer == -1:
            self += b
            return

        self.more_pistons(layer // 2)
        self.full_extend(layer, no_row_2_opt=True)

        # retract layer below (obs)
        self.retract(layer - 3)
        if layer >= 2:
            self.less_obs()

        if layer == 8:
            self.tape_ready_mode = True
        # remove pistons
        self.full_row(layer - 2)
        if layer != 8:
            for sp in range(layer // 2, -1, -1):
                self.shift_pistons(sp)
        else:
            for sp in range(3, -1, -1):
                self.shift_pistons(sp)

    def tape_in_the_phokin_piston_stack(self):
        assert self.stack_state == [False, False, True, False, True, True]
        assert not self.a_toggle
        self += [wait, sto]
        self += [fold, f, sto, a, fold, fold, fold, f, sto, a, wait] * 7
        self += [worm, e, sto, a, sto]
        self.stack_state = [False, False, True, True, True, False]

    def _add(self, *moves: Move):
        self.call_tree.add_message(" ".join(map(str, moves)))

        for move in moves:
            last_move = self.moves[-1] if self.moves else None

            if (
                (move == obs and last_move not in (wait, fold, obs, worm))
                or (
                    move == b
                    and ((last_move == a and not self.a_toggle) or last_move == obs)
                )
                or (last_move == sto and move not in (fold, sto))
            ):
                self.moves.append(wait)

            self.moves.append(move)

            if move == fold:
                self.fold_toggle = not self.fold_toggle
            if move == a:
                self.a_toggle = not self.a_toggle


def main():
    door = HipSeq10()
    door.the_whole_shebang()
    # door.stack_state[0] = True
    # door.stack_state[5] = True
    # door.pull(8)
    # door += f
    # door.tape_in_the_phokin_piston_stack()
    print(len(door.moves))
    door._write_sequence("door_meta/10x10hipnew/sequence.txt")
    door._write_call_tree("door_meta/10x10hipnew/call_tree.yaml")


if __name__ == "__main__":
    main()

"""
TODO:

- independent b decoder
- retest other rows (do like 9 for solid test, everything should be fine after that)
- post pull 8 tape shit
  - funny video
- test row 10
- closing
- full test
"""
